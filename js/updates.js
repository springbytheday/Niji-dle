...
const LINKS = [
  {
    ...
    description:
      "Spotted something off, or have an idea? Let us know through the feedback form.",
  ...
  },
  {
    ...
    description:
      "Get exclusive updates, help with testing, and talk directly with the dev.",
  ...
  },
  {
    ...
    description:
      "Share fun facts of your oshi and favorite livers which will be added in the upcoming clues update",
    url: "https://forms.gle/nWHN52Pvw6siMRTK8",
  },
  {
    label: "Color Sorter",
    title: "Liver color categorizer",
    description: "Help to categorize liver colors into different families",
    url: "/color-sorter",
  },
];
...
function buildCardHtml(update) {
  const tags = [
    `<span class="tag">${escapeHtml(TYPE_LABELS[update.type] || update.type)}</span>`,
  ];
  if (update.extraTag) {
...
function showLoadingState() {
  ...
  containers.forEach((id) => {
    ...
    if (el)
      el.innerHTML = '<p class="board-loading">Loading updates&hellip;</p>';
  });
}
...
function updateColumnToggle(status, visibleCardsInColumn) {
  ...
  button.textContent = expandedState[status]
    ? "Show less"
