...
function renderRow(guessResult, isNewest, animate) {
  ...
  const thumbnail = hasImage
    ...
    : "";
  const nameCellClass = hasImage
    ? "cell cell--name has-image"
    : "cell cell--name";
...
function buildEmojiGrid() {
  ...
  return orderedOldestFirst
    .map((g) =>
      COLUMNS.map((col) => STATUS_EMOJI[g[col.key].status] || "⬜").join(""),
    )
    .join("\n");
}
...
function renderNameLinks(liver) {
  const links = [];
  if (liver.youtube_url) {
    links.push(`<a class="end-link-icon" href="${escapeHtmlAttr(liver.youtube_url)}" target="_blank" rel="noopener noreferrer" aria-label="YouTube">
  ...
  if (liver.twitter_url) {
    links.push(`<a class="end-link-icon" href="${escapeHtmlAttr(liver.twitter_url)}" target="_blank" rel="noopener noreferrer" aria-label="X">
      <svg viewBox="0 0 24 24" width="15" height="15" fill="currentColor" aria-hidden="true"><path d="M18.9 2H22l-7.6 8.7L23 22h-6.9l-5.4-6.6L4.5 22H1.3l8.1-9.3L1 2h7l4.9 6.1L18.9 2Zm-1.2 18h1.9L7.5 4h-2l12.2 16Z"/></svg>
    </a>`);
  }
  return links.length ? `<span class="end-links">${links.join("")}</span>` : "";
}
...
function showEndState() {
  ...
  if (input) {
    ...
    input.placeholder =
      currentMode === "daily"
        ? "Today's liver has been found"
        : "You've found the liver";
  }
...
<div class="end-card">
  <p class="end-title">It was ${escapeHtml(answerliver.name)}!</p>
    ${answerliver.image_url ? `<img class="end-liver-thumb" src="${escapeHtmlAttr(answerliver.image_url)}" alt="" />` : ""}
  <p class="end-links-row">${renderNameLinks(answerliver)}</p>
...
function renderStatsBlock(stats, mode, branchSignature) {
  ...
  if (mode === "daily") {
    ...
    rows.push({
      ...
      value: stats.averageGuessesOnWins,
    });
...
function updateModeToggle(mode) {
  ...
  if (dailyTab && unlimitedTab) {
    ...
    unlimitedTab.setAttribute(
      "aria-selected",
      mode === "unlimited" ? "true" : "false",
    );
  }
...
function setupAutocomplete() {
  ...
  function clearHighlight() {
    getItems().forEach((item) =>
      item.classList.remove("suggestion-item--highlighted"),
    );
    highlightedIndex = -1;
  }
