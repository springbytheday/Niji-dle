...
function createWheel() {
	...
	chromatic.forEach((c, i) => {
		...
		if (c.name === 'Cream') div.style.color = '#333';
...
function setupBranchPicker() {
	...
	function confirmSelection() {
		...
		if (checked.length === 0) {
			if (errorEl)
				errorEl.textContent = 'Pick at least one branch to continue.';
			return;
		}
		closeBranchPicker();
...
function setupAllStatsModal() {
	...
	const clearButton = document.getElementById('allstats-clear');
	...
	function disarmClear() {
		...
		if (clearButton) {
			...
			clearButton.classList.remove('allstats-clear--armed');
		}
	}
	...
	function renderBody() {
		...
		if (clearButton) {
			clearButton.style.display = getAllUnlimitedStats().length ? '' : 'none';
		}
	}
	...
	function trapFocus(e) {
		const focusable = Array.from(dialog.querySelectorAll('button')).filter(
	...
	if (clearButton) {
		clearButton.addEventListener('click', () => {
			if (!clearArmed) {
				clearArmed = true;
				clearButton.textContent = 'Tap again to confirm';
				clearButton.classList.add('allstats-clear--armed');
				clearTimer = setTimeout(disarmClear, 4000);
				return;
			}
			disarmClear();
			if (typeof clearUnlimitedStats === 'function') clearUnlimitedStats();
			renderBody();
			// Refresh the stats block on the end-card sitting behind the modal
			if (currentMode === 'unlimited' && gameOver) showEndState();
		});
	}
...
function setupSettingsModal() {
	...
	function switchTab(targetTabId) {
		...
		panels.forEach((panel) => {
			panel.classList.toggle(
				'overlay-panel--hidden',
				panel.id !== 'settings-tab-' + targetTabId,
			);
		});
	}
