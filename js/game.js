...
async function loadlivers() {
	...
	if (error) {
		...
		showError(
			'Could not load the livers. Check your connection and try again.',
		);
		return [];
	}
	return data;
...
function getGuessablePool() {
	if (currentMode === 'daily') {
		const eligible = alllivers.filter((t) =>
			DAILY_BRANCHES.includes(String(t.branch).toLowerCase()),
		);
...
async function resolveDailyAnswer(dateKey, livers) {
	...
	if (error) {
		console.error(
			'get_or_create_daily_puzzle RPC failed, using local fallback:',
			error,
		);
	...
	if (!row) {
		console.error(
			'get_or_create_daily_puzzle returned no row, using local fallback',
		);
		return resolveDailyAnswerFallback(dateKey, livers);
	}
...
function resolveDailyAnswerFallback(dateKey, livers) {
	const eligible = livers.filter((t) =>
		DAILY_BRANCHES.includes(String(t.branch).toLowerCase()),
	);
...
function compareLiverColor(
	...
	if (g === a)
		return { status: 'partial', color: guessColor, family: guessHueFamily };
	if (areColourNeighbours(g, a))
		return {
			status: 'another_partial',
			color: guessColor,
			family: guessHueFamily,
		};
...
async function switchMode(mode, isInitialLoad = false) {
	...
	if (mode === 'daily') {
		...
		if (!resolved) {
			showError(
				'Could not determine today\u2019s puzzle. Please try again later.',
			);
			return;
		}
		...
		renderBoard();
		if (savedUnlimited) {
			const savedAnswer = alllivers.find(
				(t) => t.id === savedUnlimited.answerId,
			);
			if (savedAnswer) {
...
async function newUnlimitedRound() {
	...
	unlimitedState = {
		answer: resolveUnlimitedAnswer(getUnlimitedPool()),
		guesses: [],
		gameOver: false,
	};
	applyState(unlimitedState);
...
function submitGuess(liver) {
	...
	if (liver.id === answerliver.id) {
		...
		recordResult(
			currentMode,
			{ won: true, guessCount: guesses.length },
			getBranchSignature(selectedBranches),
		);
	}
...
// Repairs stats saved before totalWins / totalGuessesAll existed, and records
// already corrupted by merging legacy data with the new defaults.
function normalizeStats(mode, raw) {
	if (!raw || typeof raw !== 'object') return raw;
	if (raw.statsVersion >= STATS_VERSION) return raw;

	const s = { ...raw };
	const played = s.totalPlayed || 0;
	const onWins = s.totalGuessesOnWins || 0;

	if (mode === 'daily') {
		// No give-up in daily, so every played round was a win.
		s.totalWins = played;
	} else {
		// Legacy (no totalWins / totalGuessesAll), or corrupted by the bad merge.
		// In a healthy record totalGuessesAll >= totalGuessesOnWins always holds.
		const needsRepair =
			s.totalWins == null ||
			s.totalGuessesAll == null ||
			onWins > s.totalGuessesAll;
		if (needsRepair) {
			s.totalWins = played;
			s.totalGuessesAll = onWins;
		}
	}
...
function defaultStats(mode) {
	...
	base.statsVersion = STATS_VERSION;
	if (mode === 'daily') {
...
function loadUnlimitedStatsMap() {
	...
	for (const sig of Object.keys(map || {})) {
		if (map[sig] && !(map[sig].statsVersion >= STATS_VERSION)) {
			map[sig] = normalizeStats('unlimited', map[sig]);
			changed = true;
		}
	}
	if (changed) saveUnlimitedStatsMap(map);
...
function loadStats(mode, branchSignature) {
	if (mode === 'daily') {
		try {
			...
			const parsed = JSON.parse(raw);
			const fixed = normalizeStats('daily', parsed);
			if (fixed !== parsed)
				saveStats('daily', { ...defaultStats('daily'), ...fixed });
			return { ...defaultStats('daily'), ...fixed };
		} catch (e) {
...
function clearUnlimitedStats() {
	try {
		localStorage.removeItem(STATS_KEYS.unlimited);
	} catch (e) {
		console.warn('Could not clear stats:', e);
	}
}
...
function recordResult(mode, { won, guessCount = 0 }, branchSignature) {
	...
	if (mode === 'daily') {
		...
		stats.currentStreak =
			stats.lastSolvedDate === yesterday ? stats.currentStreak + 1 : 1;
...
function getStats(mode, branchSignature) {
	const stats = loadStats(mode, branchSignature);
	const averageGuessesOnWins =
		stats.totalWins > 0
			? (stats.totalGuessesOnWins / stats.totalWins).toFixed(1)
			: '—';
	const averageGuessesAll =
		stats.totalGuessesAll != null && stats.totalPlayed > 0
...
function giveUpUnlimited() {
	...
	recordResult(
		'unlimited',
		{ won: false, guessCount: guesses.length },
		getBranchSignature(selectedBranches),
	);
	saveProgress();
	renderBoard();
